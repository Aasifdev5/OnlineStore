            ...
            <div class="container">
                <div class="hero__slider--inner hero__slider--ml hero__slider--activation swiper">
                    <div class="hero__slider--wrapper swiper-wrapper">
                        ...
                        @endphp
                        @if(!empty($banners) and count($banners) > 0)
                        @foreach($banners as $row)
                        <div class="swiper-slide ">
                            <div class="hero__slider--items home1-slider1-bg">
                                <div class="slider__content">
                                    <span class="slider__subtitle text__secondary">{{$row->title1}}</span>
                                    <h2 class="slider__maintitle h1">{{$row->title2}} <br> <span
                                            class="slider__maintitle--inner__text">{{$row->title3}}</span></h2>
                                    <!--<span class="slider__price--text text__secondary">415.00/BS</span>-->
                                    <a class="primary__btn slider__btn" 
   href="{{ !empty($row->link) ? $row->link : url('shop') }}">
    {{ !empty($row->button) ? $row->button : 'Tienda' }}
        ...
        <path d="M11.8335 3.6178L8.26381 0.157332C8.21395 0.107774 8.1532 0.0681771 8.08544 0.0410843C8.01768 0.0139915 7.94441 0 7.87032 0C7.79624 0 7.72297 0.0139915 7.65521 0.0410843C7.58745 0.0681771 7.5267 0.107774 7.47684 0.157332C7.37199 0.262044 7.31393 0.39827 7.31393 0.539537C7.31393 0.680805 7.37199 0.817024 7.47684 0.921736L10.0943 3.45837H0.55625C0.405122 3.46829 0.26375 3.52959 0.160556 3.62994C0.057363 3.73029 0 3.86225 0 3.99929C0 4.13633 0.057363 4.26829 0.160556 4.36864C0.26375 4.46899 0.405122 4.53029 0.55625 4.54021H10.0927L7.47527 7.07826C7.37042 7.18298 7.31235 7.3192 7.31235 7.46047C7.31235 7.60174 7.37042 7.73796 7.47527 7.84267C7.52513 7.89223 7.58588 7.93182 7.65364 7.95892C7.7214 7.98601 7.79467 8 7.86875 8C7.94284 8 8.0161 7.98601 8.08386 7.95892C8.15162 7.93182 8.21238 7.89223 8.26223 7.84267L11.8335 4.38932C11.9406 4.28419 12 4.14649 12 4.00356C12 3.86063 11.9406 3.72293 11.8335 3.6178V3.6178Z"
            fill="currentColor" />
                                ...
                                <div class="hero__slider--layer">
                                    <img class="slider__layer--img " src="{{asset($row->image)}}"
                                        alt="slider-img">
                                </div>
                            </div>
                        </div>
    ...
    .categories__card--link:hover .categories__title,
    .categories__card--link:hover .categories__subtitle {
        color: black;
    }
</style>
            ...
            <a class="categories__card--link" href="{{ url('productbyCategory') }}/{{ $parentCategory->id }}" 
               style="display: block; text-decoration: none; transition: background-color 0.3s ease, color 0.3s ease;">
                <span class="categories__icon">
                    <img src="{{ asset($parentCategory->image) }}" style="max-width: 100%; height: auto;">
                </span>
                <h2 class="categories__title" style="transition: color 0.3s ease;">{{ $parentCategory->name }}</h2>
                <span class="categories__subtitle" style="transition: color 0.3s ease;">
                    @php
        ...
        $categories = \App\Models\Category::whereIn('id', $userCategories)->select('id', 'name')->get();
                                       @endphp
                                        ...
                                        <li class="choose__category--menu__list">
                                            ...
                                            <a class="choose__category--menu__link" href="{{ url('productbyCategory') }}/{{ $parentCategory->id }}"><strong>{{$parentCategory->name}}</strong></a>
                                        </li>
                                        @endforeach
                                    </ul>
                                </div>
                                                ...
                                                <article class="product__card">
                                                    ...
                                                    <div class="product__card--content">
                                                        ...
                                                        <div class="product__card--footer">
                                                             @if(empty($IsVariation))
                                                            <!--<a class="product__card--btn primary__btn" href="{{ url('addToCart') }}/{{ $price }}/{{ $row->id.'/1/default' }}">-->

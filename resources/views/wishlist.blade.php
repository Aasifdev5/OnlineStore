...
@section('content')
  ...
  @if(Session::has('success'))
    ...
    @php
        ...
        // Initialize the variable for product images
        $productImages = null;
        ...
        if ($product_details) {
            ...
            $productImagesCount = \App\Models\Pro_image::where('product_id', $product_details->id)
                                                        ->where('color', 'm')
                                                        ->count();
            ...
            if ($productImagesCount > 1) {
                ...
                $productImages = \App\Models\Pro_image::where('product_id', $product_details->id)
                                                      ->where('color', 'm')
                                                      ->orderBy('id', 'asc')
                                                      ->first();
            } else {
            ...
            <td class="cart__table--body__list">
                <div class="cart__product d-flex align-items-center">
                    <a href="{{ route('remove.wish', $item->id) }}"
                        class="cart__remove--btn" aria-label="remove button" type="button">
                    ...
                    <div class="cart__thumbnail">
                        <a href="{{ url('product-details/' . $product_details->slug) }}">
                            @if (!empty($productImages))
                                <img class="border-radius-5" src="{{ asset($productImages->thumbnail) }}" alt="wishlist-product">
                            @else
                                <img class="border-radius-5" src="{{ asset('product_images/' . $product_details->f_thumbnail) }}" alt="wishlist-product">
                            @endif
                        </a>
                    </div>
        ...
        <tr class="cart__table--body__items">
            <td colspan="4" class="cart__table--body__list">
                <div class="alert alert-danger" role="alert">
                    Product not found for wishlist item with ID: {{ $item->id }}
                </div>
            </td>
        </tr>
    @endif
@endforeach
                            ...
                            <br>
                           <div class="text-left">
    <a href="{{ route('clear.wishlist') }}" class="cart__summary--footer__btn primary__btn checkout pull-right">Vaciar Lista de Deseos</a>
</div>

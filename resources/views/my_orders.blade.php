        ...
        @foreach ($order->orderItems as $orderItem)
            @if ($orderItem->product) {{-- Ensure the product is not null --}}
                <tr class="account__table--body__row">
                    @if ($first)
                        <td class="account__table--body__cell" rowspan="{{ $order->orderItems->count() }}">{{ $order->id }}</td>
                        <td class="account__table--body__cell" rowspan="{{ $order->orderItems->count() }}">{{ \Carbon\Carbon::parse($order->created_at)->setTimezone('America/La_Paz')->format('F j, Y') }}</td>
                        <td class="account__table--body__cell" rowspan="{{ $order->orderItems->count() }}">

...
<script>
   document.addEventListener('DOMContentLoaded', function() {
    ...
    priceFilterForm.addEventListener('submit', function(e) {
        ...
        .then(data => {
            ...
            data.products.forEach(product => {
                ...
                productElement.className = 'product-grid';

                // Check if variations exist
                const primaryImg = product.has_variation ? product.variation_details.f_thumbnail : product.f_thumbnail;

                productElement.innerHTML = `
                    <div class="product-card">
                        <a href="/product-details/${product.slug}" class="product-link">
                            <img src="/product_images/${primaryImg}" alt="${product.title}" class="product-image">
    ...
    @php
        ...
        $categories = \App\Models\Category::whereIn('id', $userCategories)->select('id', 'name', 'image')->get();
    @endphp
    @foreach ($categories as $parentCategory)
        <li class="categories__shop--card" style="list-style: none;">
            <a class="categories__shop--card__link" href="{{ url('productbyCategory') }}/{{ $parentCategory->id }}" 
                ...
                <div class="categories__thumbnail mb-15">
                    <img class="categories__thumbnail--img" src="{{ asset($parentCategory->image) }}" alt="categories-img">
                </div>
                <div class="categories__content">
                    <h2 class="categories__content--title" style="transition: color 0.3s ease;">{{$parentCategory->name}}</h2>
                    <span class="categories__content--subtitle" style="transition: color 0.3s ease;">
                        @php

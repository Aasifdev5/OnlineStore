...
@section('content')
                                    ...
                                    <div class="card">
                                        <div class="card-body ecommerce-icons text-center"><i
                                            ...
                                            <h4 class="font-primary mb-0 counter">
                                                @php
    $total_earning = \App\Models\Payment::where('accepted', 1)->sum('amount');
   $total_orders = \App\Models\Order::count();
    ...
    $current_month_orders = \App\Models\Order::whereMonth('created_at', $currentMonth)
                     ...
                     ->count();
    $current_month_earning = \App\Models\Payment::where('accepted', 1)
                        ->whereMonth('created_at', $currentMonth)
                        ->whereYear('created_at', $currentYear)
                        ->sum('amount');
                        $today = date('Y-m-d');
                        $today_orders = \App\Models\Order::whereDate('created_at', $today)->count();
    $today_earning = \App\Models\Payment::where('accepted', 1)

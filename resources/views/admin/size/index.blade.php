                            ...
                            <div class="breadcrumb__content__right">
                                <nav aria-label="breadcrumb">
                                    <ul class="breadcrumb">
                                        ...
                                        <li class="breadcrumb-item active" aria-current="page">{{ __('Size') }}</li>
                                    </ul>
                                </nav>
                            </div>
                        </div>
                    ...
                    <div class="col-md-12">
                        <div class="card">
                            <div class="card-header">
                                <h2>{{ __('Size List') }}</h2>
                                <a href="{{ route('size.create') }}" class="btn btn-success btn-sm pull-right"> <i
                                        class="fa fa-plus"></i> {{ __('Add Size') }} </a>
                            </div>
                            <div class="card-body">
                                                ...
                                                <tr class="removable-item">
                                                    ...
                                                    <td>
                                                        <div class="action__buttons">
                                                            <a href="{{ route('size.edit', [$category->id]) }}"
                                                                title="Edit"
                                                            ...
                                                            <a href="javascript:void(0);" title="Delete"
                                                                ...
                                                                data-url="{{ route('size.delete', $category->id) }}">
            ...
            if (confirm('{{ trans('do you want to delete') }}')) { // Confirmation prompt with translation
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            toastr.success("size successfully deleted.", "", {
                                onHidden: function() {
                                    window.location
                            ...
                            toastr.error("Failed to delete size.");
                        }
                    },
                    error: function(xhr, status, error) {
                        console.error(xhr.responseText); // Log errors to console for debugging
                        toastr.error("Failed to delete size. Please try again later.");
                    }
                });
            }
        });

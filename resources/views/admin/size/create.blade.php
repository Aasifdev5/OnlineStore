                            ...
                            <div class="col-md-12">
                                <div class="form-vertical__item bg-style">
                                    <div class="item-top mb-30">
                                        <h2>{{ __('Add New size') }}</h2>
                                    </div>
                                    <form id="Form" action="{{ route('size.store') }}" method="POST">
                                        @csrf
        ...
        $('#Form').submit(function(e) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        toastr.success("size successfully created.", "", {
                            onHidden: function() {
                                window.location.href = "{{ route('size.index') }}";
                            }
                        });
                    } else {
                        toastr.error("Failed to create size.");
                    }
                },
                error: function(xhr, status, error) {
                    console.error(xhr.responseText);
                    toastr.error("Failed to create size. Please try again later.");
                }
            });
        });
    </script>
